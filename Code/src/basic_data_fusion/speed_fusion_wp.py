...
class SpeedFusionAggregatorWP:
    ...
    def get_waypoint_speed(self):
        ...
        """
        df_waypoint = pd.read_sql(query, self.conn)

        if df_waypoint.empty:
            raise ValueError("No data found in map_matching and waypoint tables.")

        df_waypoint["link_id"] = df_waypoint["link_id"].astype(str)
        df_waypoint = df_waypoint[df_waypoint["link_id"].isin(self.valid_link_ids)]

        df_waypoint["time_bin"] = pd.to_datetime(df_waypoint["formatted_local_time"]).dt.floor("15min")
        df_avg_speed = df_waypoint.groupby(["link_id", "time_bin"])["speed_mph"].mean().reset_index()

        df_avg_speed.rename(columns={"speed_mph": "speed"}, inplace=True)
    ...
    def plot_link_speed_profiles(self, df_final_speed):
        ...
        output_dir = os.path.join(self.output_folder, "basic_data_fusion", "link_speed_profile_wp")
        ...
        for link_id in unique_links:
            for is_weekend, label in [(0, "Weekday"), (1, "Weekend")]:
                ...
                filename = f"speed_profile_link_{link_id}_{label.lower()}_wp.png"
    ...
    def plot_heatmap(self, df_final_speed, grouped_links):
        ...
        output_dir = os.path.join(self.output_folder, "basic_data_fusion", "link_cbi_heatmap_wp")
        os.makedirs(output_dir, exist_ok=True)
        ...
        for _, row in grouped_links.iterrows():
            ...
            for is_weekend, label in [(0, "Weekday"), (1, "Weekend")]:
                ...
                filename = f"link_cbi_heatmap_corridor_{group_id}_{label.lower()}_wp.png"
                filepath = os.path.join(output_dir, filename)
                plt.savefig(filepath, dpi=300)
                plt.close()
    ...
    def run(self):
        ...
        if self.stop_event and self.stop_event.is_set():
            return
        df_waypoint_speed = self.get_waypoint_speed()

        df_final_speed = pd.concat([df_mapped_tmc_speed, df_waypoint_speed], ignore_index=True)
        ...
        output_dir = os.path.join(self.output_folder, "basic_data_fusion")
        os.makedirs(output_dir, exist_ok=True)
        output_file = os.path.join(output_dir, "speed_fusion_wp.csv")
...
if __name__ == "__main__":
    ...
    speed_aggregator_wp = SpeedFusionAggregatorWP(DEFAULT_DATABASE_PATH, DEFAULT_OUTPUT_FOLDER)
    speed_aggregator_wp.run()
