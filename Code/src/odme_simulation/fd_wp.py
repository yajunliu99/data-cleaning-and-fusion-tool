...
class plot_calibration_results():
    ...
    def plot_qk(self):
        ...
        plt.title('Flow vs. density Waypoint', fontsize=18)
        save_path = os.path.join(self.output_folder, "flow_vs_density_wp.png")
    ...
    def plot_vk(self):
        ...
        plt.ylim((0, 100))
        plt.legend(loc='upper right', fontsize=14)
        plt.title('Speed vs. density Waypoint', fontsize=18)
        save_path = os.path.join(self.output_folder, "speed_vs_density_wp.png")
        fig.savefig(save_path, dpi=400, bbox_inches='tight')
        plt.close(fig)
    ...
    def plot_vq(self):
        ...
        plt.ylim((0, 100))
        plt.legend(loc='upper right', fontsize=14)
        plt.title('Speed vs. flow Waypoint', fontsize=18)
        save_path = os.path.join(self.output_folder, "speed_vs_flow_wp.png")
        fig.savefig(save_path, dpi=400, bbox_inches='tight')
        plt.close(fig)
...
class DataAggregator:
    ...
    def compute_penetration_rate(self, df_lane, df_map):
        """Compute penetration rates based on lane readings and map matching data."""
        if self.stop_event and self.stop_event.is_set():
    ...
    def get_map_matching_readings(self):
        """Retrieve speed data from map_matching and waypoint tables, excluding invalid link_id."""
        if self.stop_event and self.stop_event.is_set():
            print("Stopping get_map_matching_readings")
            return pd.DataFrame()

        query = """
        SELECT m.link_id, strftime('%Y-%m-%d %H:%M:%S', m.time) AS formatted_time, 
               strftime('%Y-%m-%d %H:%M:%S', w.local_time) AS formatted_local_time, 
               w.speed_mph 
        FROM map_matching m
        JOIN waypoint w ON m.agent_id = w.journey_id AND formatted_time = formatted_local_time
        """
        df_map_matching = pd.read_sql(query, self.conn)

        if df_map_matching.empty:
            raise ValueError("No data found in map_matching and waypoint tables.")

        df_map_matching["time_bin"] = pd.to_datetime(df_map_matching["formatted_local_time"]).dt.floor("15min")
        df_avg_speed = df_map_matching.groupby(["link_id", "time_bin"])["speed_mph"].mean().reset_index()
        df_avg_speed = df_avg_speed.rename(columns={"speed_mph": "speed"})
    ...
    def get_link_volume(self):
        ...
        query = "SELECT link_id, agent_id, time FROM map_matching"
        ...
        # Aggregate unique agent_id counts per 15-minute interval
        df_link_volume = df_volume.groupby(["link_id", "time_bin"])["agent_id"].nunique().reset_index()
        df_link_volume.rename(columns={"agent_id": "volume"}, inplace=True)

        df_lane = self.get_lane_readings()
        df_map = self.get_map_matching_volumes()
        self.compute_penetration_rate(df_lane, df_map)
        ...
        df_link_volume = df_link_volume.merge(self.penetration_rate_df[["time_of_day", "map_penetration_rate"]], on="time_of_day", how="left")

        # Adjust volume using penetration rate
        df_link_volume["adjusted_volume"] = df_link_volume["volume"] / df_link_volume["map_penetration_rate"]
...
class FDCalibratorWP:
    def __init__(self, database_path, output_folder, stop_event=None):
        """Initialize the Fundamental Diagram Calibrator."""
        self.database_path = database_path
        self.output_folder = output_folder
        self.output_path = os.path.join(self.output_folder, "odme_simulation", "fd_wp")
    ...
    def compute_fundamental_diagram_data(self):
        ...
        aggregator = DataAggregator(self.database_path, self.output_folder, stop_event=self.stop_event)
        ...
        df_fd.to_csv(os.path.join(self.output_path, "fundamental_diagram_data_wp.csv"), index=False)
...
if __name__ == "__main__":
    ...
    fd_calibrator_wp = FDCalibratorWP(DEFAULT_DATABASE_PATH, DEFAULT_OUTPUT_FOLDER)
    fd_calibrator_wp.run()
