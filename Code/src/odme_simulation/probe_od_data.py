...
class ProbeODAnalyzer:
    ...
    def extract_od_volume(self, filter_day_parts, output_filename):
        ...
        od_df = od_df[(od_df["Day_Type"] == 0) & (od_df["Day_Part"].isin(filter_day_parts))]
        ...
        od_matrix = od_matrix.reindex(full_index, fill_value=0).reset_index()
    ...
    def run(self):
        ...
        if self.stop_event and self.stop_event.is_set():
            return
        self.extract_od_volume([0], "probe_od_volume_all_day.csv")

        self.extract_od_volume([33, 34, 35, 36], "probe_od_volume_8_9am.csv")

        print("Probe OD Data Extraction Completed.")
        self.conn.close()
...
if __name__ == "__main__":
    ...
    probe_od_analyzer = ProbeODAnalyzer(DEFAULT_DATABASE_PATH, DEFAULT_OUTPUT_FOLDER)
    probe_od_analyzer.run()