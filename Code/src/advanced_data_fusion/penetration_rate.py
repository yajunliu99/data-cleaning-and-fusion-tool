...
class PenetrationRateAnalyzer:
    ...
    def compute_penetration_rate(self, df_lane, df_map, df_traj):
        ...
        df_merged = df_merged.merge(df_traj, on=["time_of_day", "is_weekend"], how="left")
    ...
    def plot_penetration_rate(self, df_penetration):
        ...
        output_dir = os.path.join(self.output_folder, "advanced_data_fusion", "penetration_rate")
        ...
        for is_weekend, label in [(0, "Weekday"), (1, "Weekend")]:
            ...
            filename = f"penetration_rate_{label.lower()}.png"
    ...
    def run(self):
        ...
        if self.stop_event and self.stop_event.is_set():
            return
        df_penetration = self.compute_penetration_rate(df_lane, df_map, df_traj)
        # print(df_penetration)

        # Save penetration rate data
        if self.stop_event and self.stop_event.is_set():
            return
        penetration_rate_path = os.path.join(self.output_folder, "advanced_data_fusion", "penetration_rates.csv")
...
if __name__ == "__main__":
    ...
    penetrationrate_analyzer = PenetrationRateAnalyzer(DEFAULT_DATABASE_PATH, DEFAULT_OUTPUT_FOLDER)
    penetrationrate_analyzer.run()
